...
def main():
    batch_size = 1
    train_dir = '/home/animesh/storage/TransMorph_Transformer_for_Medical_Image_Registration/data/OASIS_L2R_2021_task03/All/'
    val_dir = '/home/animesh/storage/TransMorph_Transformer_for_Medical_Image_Registration/data/OASIS_L2R_2021_task03/Test/'
    ...
    train_set = datasets.AbdomenCTCTDataset(data_path="../../data/AbdomenCTCT", transforms=train_composed)
    val_set = datasets.AbdomenCTCTInferDataset(data_path="../../data/AbdomenCTCT", transforms=val_composed)
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    val_loader = DataLoader(val_set, batch_size=1, shuffle=False, num_workers=4, pin_memory=True, drop_last=True)

    optimizer = optim.Adam(model.parameters(), lr=updated_lr, weight_decay=0, amsgrad=True)
    criterion_ncc = losses.NCC_vxm()
    criterion_dsc = losses.DiceLoss(num_class=14)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            x, y = x.permute(0, 1, 3, 2, 4), y.permute(0, 1, 3, 2, 4)
            x_seg, y_seg = x_seg.permute(0, 1, 3, 2, 4), y_seg.permute(0, 1, 3, 2, 4)
            
            x_seg_oh = nn.functional.one_hot(x_seg.long(), num_classes=14)
            x_seg_oh = torch.squeeze(x_seg_oh, 1)
            x_seg_oh = x_seg_oh.contiguous()
            # x_seg_oh = x_seg_oh.permute(0, 4, 1, 2, 3)
...
def mk_grid_img(grid_step, line_thickness=1, grid_sz=(160, 192, 256)):
